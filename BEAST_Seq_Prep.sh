...
###### Additional analyses to corroborate the previous trees ######
...
python scripts/remove_ts.py aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT.fasta aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only.fasta &> aln_only_tv/BT_dataset.log &

# remove columns with missing data, Ns or monomorphic sites throughout 

snp-sites aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only.fasta > aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only_poly.fasta

mkdir -p MLE_trees_only_tv

# run tree with a lewis SNP ascertainment bias correction 

raxmlHPC-PTHREADS -f a -T 10 -x 12345 -k -# 100 -p 12345 -m ASC_GTRGAMMA --asc-corr lewis -o NC_002641.1 -s aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only_poly.fasta -n all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only_poly -w /media/jbod/home/antony/Marek-Capture/masked_data/NEW_ANAL_2021/MLE_trees_only_tv &> MLE_trees_only_tv/log-make_RAxML_TREE.txt &



# 15) Build an MLE tree with HVT as an outgroup BUT with only modern data

mkdir -p MLE_trees_modern_only

raxmlHPC-PTHREADS -f a -T 10 -x 12345 -k -# 100 -p 12345 -m GTRGAMMA -o NC_002641.1 -s aln_files/all_modern_plus_HVT_clean_masked_aln.fasta -n all_modern_plus_HVT_clean_masked_aln -w /media/jbod/home/antony/Marek-Capture/masked_data/NEW_ANAL_2021/MLE_trees_modern_only &> MLE_trees_modern_only/log-make_BEAST_TREE.txt &


# 16) Calculate dXY for BEAST dataset but with no TS 

# input aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only_poly.fasta

mkdir -p dXY_tv_only; cd dXY_tv_only; 
ln -s ../aln_only_tv/all_modern_plus_HVT_clean_masked_aln_anc_BT_tv_only_poly.fasta .; 
cd ..

# run dXY 

rscript dXY_calculation.r


# 17) Plot the tMRCA empirical distribution function 

rscript tMRCA_EDF.r



# 18) Calculate the minor read frequency in multiallelic SNP sites per sample

for x in trimmed_bams_snp/*_adRm_sorted_rmdup_m_trim_RG.bam; 
	do echo $x; 
	samtools mpileup -a $x | grep '78996\|131137\|134260\|134271\|136264\|137891\|138971\|141356\|152323\|153320\|154148' > 
	`basename -s _adRm_sorted_rmdup_m_trim_RG.bam $x`_pileup_mini.txt; 
	python scripts/count_minor_read_freq.py  `basename -s _adRm_sorted_rmdup_m_trim_RG.bam $x`_pileup_mini.txt; 
done
